...
Among the variations, we used a version of R called “time-varying” reproduction number, which concerns how disease transmissibility evolves over time, rather than the inherent biological characteristics of a pathogen. As infectious cases accumulate over time, cases grow faster with more cases from the susceptible, exposed and recovered populations, yet the speed of growth would slow down over time within a population of a finite size. Therefore, Rt is always smaller than R0.
...
* As $I_t$ is a stochastic process, $R_t$ is as well. At time $t$, $R_t$ is right censored due to the unreported cases for 1) the delay in reporting, 2) the incubation period of a new case, 3) the generation time, a time between infection events. These three parameters are additionally considered in {EpiNow2} by [@abbott2020estimating], which contribute to the better uncertainty estimates for Rt. 

* We used estimate_R() function with a ``parametric_si” option assuming a gamma prior with a mean of 4 days, a standard deviation of 4.75 days, and a 7-day smoothing window  suggested by [@abbott2020estimating],  [@Basu2020]. However, Bayesian estimates give unreasonably small variations (sd < 0.01) for recent data where Rt is lingering around 1 with a large number of new cases.    
...
Doubling time estimates the time it takes for the number of cases in the future to be twice what it is today, based on the current growth rate $r$ estimated by new cases within a window size $\tau$, in our case over the past week. Given that a growth rate of today reflects the data of previous 7 days, doubling time relates past observations to a prediction for the future. 
...
where $X_t$ is cumulative cases up to day $t$.  While the growth rate $r$ represents an instantaneous rate of change in cases for each day, Levitt’s statistic calculates the change in cases relative to cumulative cases, which shows how the overall number of cases is changing, or the first of change. If $H(t) < 0$, cases are decreasing. If $H(t) > 0$, cases are increasing. 

We compute the Levitt statistic for deaths only, following the suggestion by [@raman2020will]. When the number of recorded incident cases is capped by testing capacity, an incidence series does not capture the epidemic growth accurately. Deaths are less prone to this type of confounding effect, where testing capacity causes censoring of cases.   
